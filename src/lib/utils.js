...
export async function fetchData(latlngs, osmGeoJSONStore, setSouthWest, setNorthEast) {
    if (!latlngs) {
...
export async function fetchTilesAndRenderCanvas(latlngs, southWest, northEast, selectedLayer) {
    ...
    const bottomRightPixel = latLngToPixel(southWest.lat, northEast.lng, zoomLevel);
    ...
    const buildings = geo.features
        ...
        .flatMap((feature) => {
            ...
            if (feature.geometry.type === "Polygon") {
                return createBuildingGeometry(coordinates, referencePoint, feature.properties);
...
function createBuildingGeometry(coordinates, referencePoint, properties) {
    ...
    shape.holes = coordinates.slice(1).map((hole) => getShapeFromCoordinates(hole, referencePoint));
...
function createClippingPlane(latlngs, referencePoint) {
    ...
    const planeShape = new THREE.Shape(convertedPoints.map((coord) => new THREE.Vector2(coord.x, coord.y)));
    const planeSize = 2;
